...
import XwyaCarrierTypeDialog from '@/components/XwyaCarrierTypeDialog'
...
const items:FormItemsProps[] = [{ type: 'input', item: { label: '貨物名稱', name: 'name' }, content: { placeholder: '請輸入貨物名稱' } }]
const pageData = [
...
const CarrierType = () => {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex gap-2 cursor-pointer">
             <XwyaCarrierTypeDialog title='編輯承運車輛' id={payment.id}>
              <span>修改</span>
              </XwyaCarrierTypeDialog>
            <XwyaPopover onConfirm={onConfirm} content={{ side: "top" }} text='確認刪除該承運車輛嗎？'><span className=" text-red-600">刪除</span></XwyaPopover>
          </div>
        )
      },
    },
  ...
  return (
    <Layout  page="承運管理" pathKey="/carrierType">
      <XwyaForm items={items} row={5} form={form} onFinish={onFinish}>
        <div className=' flex gap-2'>
          ...
          <XwyaCarrierTypeDialog title='新增承運車輛'>
