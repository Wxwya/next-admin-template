...
const AppSidebar = ({ items, onChange = () => { }, selectedKey, iconActiveColor, iconColor,menuGroupKeys }: AppSidebar) => {
  const config = {
    iconActiveColor,
    iconColor,
    onChange,
  ...
  return (
    <Sidebar collapsible="icon"  variant="sidebar">
            ...
            <SidebarMenu>
              {items.map((item) =>
                item.routes && item.routes.length ? (
                  <SubMenuItem key={item.path} {...config} {...item} />
                ) : (
                  <MenuItem key={item.path} {...config} {...item} />
                )
              )}
            </SidebarMenu>
          </SidebarGroupContent>
      ...
      {sidebarFoot ? <SidebarFooter><AppSidebarFoot/></SidebarFooter>:null} 
    </Sidebar>
  )
}
